...
describe('ProductoService', () => {
	...
	describe('Boundary Testing - Valores límite y normalización', () => {
		describe('getProductos', () => {
		...
		describe('postProducto', () => {
			...
			it('retorna lista vacía cuando se filtra por categorías sin coincidencias', async () => {
				;(mockProductoRepository.findByCategorias as jest.Mock).mockResolvedValueOnce([])

				const result = await productoService.getProductosPorCategorias('DEPORTES')

				expect(result).toEqual([])
			})

			it('retorna lista vacía si no hay coincidencias al buscar por nombre', async () => {
				;(mockProductoRepository.findByNombre as jest.Mock).mockResolvedValueOnce([])

				const result = await productoService.getProductosPorNombre('NoExiste')
